...
The clip inspector and Video lab also expose optional Windows-compatible Sol-H3 acceleration through the installed `BSAI-ComfyUI-Sol-H3` node. Enable **Sol-H3 acceleration** to use the sparse CUDA attention path, then adjust `tau`, minimum tokens, sampling start/end, sink conditioning, Morton ordering, and QKV memory reuse. The default workflow remains unchanged when the switch is off. Sol-H3 requests are rejected early if the ComfyUI backend does not expose `BSAI_SolH3_Loader`.
